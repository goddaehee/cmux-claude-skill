...
| `cmux new-workspace [--name <title>] [--cwd <path>] [--command "cmd"]` | 워크스페이스 생성 |
| `cmux ssh <dest> [--name <title>] [--port <n>] [--identity <path>] [--no-focus]` | SSH 워크스페이스 생성 (원격 프록시 포함) |
...
| `cmux workspace-action --action <name> [--workspace <ref>] [--color <name\|#hex>]` | 액션: pin, unpin, close-others, close-above/below, move-up/down/top, mark-read/unread, **set-color**, **clear-color** |

> **네임드 컬러 (16종):** Red, Crimson, Orange, Amber, Olive, Green, Teal, Aqua, Blue, Navy, Indigo, Purple, Magenta, Rose, Brown, Charcoal
> ```bash
> cmux workspace-action --workspace workspace:1 --action set-color --color Blue
> cmux workspace-action --action set-color --color "#C0392B"  # HEX도 가능
> cmux workspace-action --action clear-color                   # 색상 제거
> ```
...
| `cmux omo [opencode-args...]` | OpenCode 통합 |
| `cmux codex <install-hooks\|uninstall-hooks>` | Codex CLI 훅 관리 |
| `cmux remote-daemon-status [--os <os>] [--arch <arch>]` | 원격 데몬 상태 확인 |
